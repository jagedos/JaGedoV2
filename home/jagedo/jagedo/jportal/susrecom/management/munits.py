...
from items.models import Categories, Products, Brands, Munits
...
def index(request):
  ...
  template = loader.get_template('items/units.html')
  context = {
    'items': items,
    'cats': cats,
    'brands': brands,
...
def edita(request, id):
  mymember = model_to_dict(Munits.objects.get(id=id))
...
def updaterecord(request):
  ...
  member = Munits.objects.get(id=id)
  member.name = name
  member.save()
