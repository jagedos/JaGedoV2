...
class EProductsListJson(BaseDatatableView):
    ...
    def render_column(self, row, column):
        if column == "cover_image":
            ...
            return super(EProductsListJson, self).render_column(row, column)
...
def index(request, uidb64):
    ...
    template = loader.get_template("items/items/products.html")
    context = {
        "items": items,
        "products": products,
        "id": id,
...
def product_filter(request):
    ...
    template = loader.get_template("items/items/filter.html")
    context = {
        "shops": shops,
    }
    return HttpResponse(template.render(context, request))
